        ...
        try {
          ...
          if (membership != null) {
            if (membership.role == 'PROJECT_ADMIN') {
              ...
              res.status(403).json({ success: false, message: 'You need to be PROJECT_ADMIN role to do this' });
            }
          } else {
        ...
        try {
          ...
          if (membership != null) {
            if (
              ...
              res.status(403).json({
                success: false,
                message: 'You need to be PROJECT_VIEWER or  PROJECT_MEMBER or PROJECT_ADMIN role to do this',
              });
            }
          } else {
            res.status(403).json({ success: false, message: 'You are not a member of this project' });
