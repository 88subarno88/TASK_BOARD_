...
export async function uploadAvatar(req: Request, res: Response) {
  try {
    ...
    if (!avatar.startsWith('data:image/')) {
      res.status(400).json({
        success: false,
        message: 'Invalid image format.Image must be base64 image data',
      });
      return;
    }
    ...
    if (avatar.length > 2 * 1024 * 1024) {
      res.status(400).json({
        success: false,
        message: 'Avatar is too large. Maximum 2MB',
      });
      return;
    }

    const user = await userService.updateProfile(userId as string, { avatarUrl: avatar });
    ...
    res.status(500).json({ success: false, message: 'ERROR: Couldnot uploadAvatar' });
  }
}
